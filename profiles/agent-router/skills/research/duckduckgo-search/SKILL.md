...
- **VPS IP blocking**: DuckDuckGo may silently block requests from cloud/VPS IP ranges — the CLI exits 0 with zero output, no error message. If even a trivial query (`ddgs text -q "python" -m 1`) returns empty, the IP is blocked. Fall back to the browser tool or another search path.
- **`site:` operator unreliable**: The `site:x.com` and `site:twitter.com` filters often return zero results even when the base query would succeed. Do not rely on `site:` with `ddgs` for Twitter/X research. Use twitter-research-no-api instead.
