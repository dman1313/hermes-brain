...
#### GEPA (Genetic-Pareto Prompt Evolution) — DSPy 3.2+

Reflective prompt evolution that reads execution traces to understand *why* things fail, then proposes targeted mutations. ICLR 2026 Oral. Installed via `dspy>=3.2.0` with `gepa>=0.0.27`.
...
Also available as `dspy.GEPA(...)` shorthand in DSPy 3.2+.

Used by: [hermes-agent-self-evolution](https://github.com/NousResearch/hermes-agent-self-evolution) — installed at `~/hermes-agent-self-evolution/` on this system. Evolve skills with:
```bash
cd ~/hermes-agent-self-evolution
python -m evolution.skills.evolve_skill --skill github-code-review --iterations 10
```

#### MIPROv2 (fallback when GEPA unavailable)
...
## LM Provider Configuration (DSPy 3.x / litellm)

DSPy 3.x uses the unified `dspy.LM("provider/model")` API backed by litellm — the old `dspy.Claude()` / `dspy.OpenAI()` constructors are no longer the standard path.
...
lm = dspy.LM("deepseek/deepseek-v4-pro")
lm = dspy.LM("gemini/gemini-2.5-pro")

dspy.configure(lm=lm)
```

API keys are auto-detected from environment variables (OPENAI_API_KEY, ANTHROPIC_API_KEY, DEEPSEEK_API_KEY, etc.) by litellm.

### DeepSeek v4 — critical pitfall

litellm auto-routes `deepseek/deepseek-v4-pro` and `deepseek/deepseek-v4-flash` to `https://api.deepseek.com/beta` which returns 401. DeepSeek v4 models use the standard endpoint. Pass `api_base` explicitly:
...
lm = dspy.LM(
    ...
    api_base="https://api.deepseek.com",  # NOT /beta
)
dspy.configure(lm=lm)
```

Also set `DEEPSEEK_API_BASE=https://api.deepseek.com` in `~/.hermes/.env` so litellm picks it up automatically for future runs.
...
```python
cheap_lm = dspy.LM("deepseek/deepseek-v4-pro")
strong_lm = dspy.LM("anthropic/claude-sonnet-4-20250514")
...
### Legacy constructors (still work, not preferred)
