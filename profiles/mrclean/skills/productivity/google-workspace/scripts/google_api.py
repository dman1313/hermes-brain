...
def drive_upload(args):
    ...
    # Search for existing file with same name to replace
    existing = service.files().list(
        q=f"name = '{args.name}' and trashed = false",
        fields="files(id, name)",
    ).execute().get("files", [])

    media = MediaFileUpload(args.file, mimetype=args.mimetype or "application/gzip", resumable=True)
    if existing:
...
def drive_delete(args):
    """Trash files matching a name query (keeps only latest)."""
    service = build_service("drive", "v3")
    results = service.files().list(
        q=f"name contains '{args.name}' and trashed = false",
        orderBy="modifiedTime desc",
        fields="files(id, name, modifiedTime)",
    ).execute().get("files", [])

    if args.keep and len(results) > args.keep:
        for f in results[args.keep:]:
            service.files().update(fileId=f["id"], body={"trashed": True}).execute()
            print(f"Trashed: {f['name']} ({f['id']})", file=sys.stderr)

    print(json.dumps({"deleted": max(0, len(results) - (args.keep or 0))}))
...
def main():
    ...
    p.add_argument("--name", required=True, help="Name for the file in Drive")
    p.add_argument("--mimetype", default="application/gzip", help="MIME type (default: application/gzip)")
    p.set_defaults(func=drive_upload)

    p = drv_sub.add_parser("delete")
    p.add_argument("name", help="Name substring to match files for deletion")
    p.add_argument("--keep", type=int, default=0, help="Keep N most recent matching files")
