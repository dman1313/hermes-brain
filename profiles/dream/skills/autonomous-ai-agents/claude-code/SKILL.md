...
16. **Non-Anthropic backends consistently time out** — Setting `ANTHROPIC_BASE_URL` to third-party proxies (Kimi via `api.kimi.com/coding`, GLM 5.1 via Z.AI `api.z.ai/api/paas/v4` or `api.z.ai/api/coding/paas/v4`) with `ANTHROPIC_API_KEY` set to the proxy key results in silent 30s timeouts with no output. Claude Code CLI v2.x appears to perform startup checks (OAuth validation, plugin discovery) against the base URL that non-Anthropic backends don't handle. The smoke test produces empty output or hangs. **Do not retry** — if the smoke test fails on a non-Anthropic backend, fall back to building directly (write_file + delegate_task) or use Goose CLI (`goose`) which supports multi-provider backends natively. Same-day (2026-05-08) failures confirmed against: Kimi 2.6, GLM 5.1 (both general and coding Z.AI endpoints).
