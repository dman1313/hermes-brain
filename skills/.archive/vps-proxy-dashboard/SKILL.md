...
## Caddy Reverse Proxy (Dedicated Port)

When the backend is a modern framework (Next.js, etc.) that won't work behind a subpath, add a dedicated server block on a new port:
...
Avoid `tee` with `&` in comments (shell background parser catches it). Write to `/tmp/` then `sudo cp`.

See `references/caddy-reverse-proxy.md` for subpath routing, Host header quirks, Next.js dev server caveats, and hairpin NAT.
...
### VPS Hairpin NAT
Many VPS providers block self-connections to the public IP (hairpin routing). `curl http://<public-ip>:PORT` times out from inside the VPS. Always test via `localhost` from inside. Verify external access from a browser outside, or use [reqbin.com](https://reqbin.com).

### Next.js Dev Server Behind Subpath Reverse Proxy
Next.js Turbopack dev server returns `Content-Length: 0` when proxied behind a subpath (`handle_path /prefix`). It needs the original Host header to SSR correctly. Use a dedicated port instead of subpath routing for Next.js apps. See `references/caddy-reverse-proxy.md` for details.

### Node.js Version Pinning with fnm + corepack
When a project requires an exact Node.js version (e.g., Node ~24, pnpm 10.33.x):

```bash
# Install fnm
curl -fsSL https://fnm.vercel.app/install | bash
# Source it (or open new shell)
...
For systemd services that need fnm, use a wrapper script at the project root that sources fnm before `exec`-ing the actual command. Do not try to inline fnm environment setup in systemd `ExecStart=`.
