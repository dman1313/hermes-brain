...
xurl supports two other auth methods that are simpler than OAuth 2.0 PKCE — no browser flow required.

### OAuth 1.0a (User Context)

If you have consumer key/secret + access token/secret from the X Developer Portal:
...
OAuth 1.0a is saved to the `default` app profile. Verify with `xurl whoami`. This auth type can access user-context endpoints (timeline, mentions, posting, DMs) and is the best all-around choice if you have the four keys.

### App-Only Bearer Token

Bearer tokens provide app-level access (no user context). Useful for search and public data:
...
- `xurl whoami` → 403 (no user context)
- `xurl user @handle` → may fail depending on endpoint requirements
- User-scoped actions (post, follow, DM, like) → unavailable
- Best for: search, public user lookups, raw API calls to public endpoints

**Forcing a specific auth type:** Use `--auth` to override the default:
```bash
xurl --auth oauth1 whoami       # force OAuth 1.0a
xurl --auth app search "query"  # force bearer token
```

Multiple auth methods can coexist — `xurl auth status` shows all configured types for the default app.

---

## ⚠️ API Credits Required

The X API uses a **pay-per-use** pricing model. Even with valid credentials, **all endpoints will fail with `CreditsDepleted` if your developer account has a $0 balance.**

**Before troubleshooting any other error, check your credit balance:**
- Go to [developer.x.com](https://developer.x.com/en/portal/dashboard) → Billing
- Minimum top-up: **$5**
- Credits deplete per-request; read endpoints cost less than write endpoints

If you see `"detail":"Your enrolled account [...] does not have any credits to fulfill this request."`, the fix is adding credits — not changing auth or credentials.
...
- **Cost**: X API access is typically paid for meaningful usage. Many failures are plan/permission problems, not code problems.\n- **No reliable free path for Twitter data**: Cookie-based scraping is fragile (cookies expire, CAPTCHAs, account risk). Public Nitter/alternative frontend instances are mostly dead or rate-limited. Self-hosting Nitter requires Docker + daily guest token refresh — high maintenance. For free Twitter search, use DuckDuckGo with `site:x.com` queries (`ddgs text -q "site:x.com from:account" -m 5`). This returns tweet titles/snippets but not structured engagement data.
