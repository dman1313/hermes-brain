...
- **Auth:** Claude Code requires API-key billing, NOT a Claude Pro subscription. Claude Pro ($20/mo) only works through claude.ai browser chat — it will NOT authenticate the CLI. You need one of:
...
- **Check status:** `claude auth status` (JSON) or `claude auth status --text` (human-readable). WARNING: this can show "logged in" even when the CLI can't make API calls (Pro subscription mismatch). Always follow up with the smoke test in Pre-Flight Check.
...
Always verify Claude is ready before attempting any task. Skipping this wastes a full 600s timeout attempt. Run **both** checks — `auth status` alone can lie.
...
| Output | Action |
|--------|--------|
| `Logged in as ...` | ✅ Proceed to Check 2 |
| `Not logged in` | ❌ Stop — tell user to run `claude auth login --console` |
| `command not found` | ❌ Stop — Claude CLI not installed |
...
| Output | Action |
|--------|--------|
| `success` | ✅ Claude Code is fully operational |
| `error` / 401 / `Please run /login` | ❌ Auth mismatch — see pitfall #15 |
| Timeout / empty | ⚠️ May work but slow — proceed cautiously |

**If not logged in:** Do NOT attempt to pipe `ANTHROPIC_API_KEY` into `claude auth login --console` yourself — this requires interactive TTY confirmation. Simply tell the user to run it themselves. The `--console` flag accepts an API key from stdin *only* when piped interactively from a real terminal, not from a script or agent process.

## Fallback: Hermes subagent via delegate_task

If Claude Code is unavailable (not installed, not authenticated, or times out), use Hermes's own `delegate_task` as a fallback code reviewer. It will be slower and less thorough than Claude, but works:

```
delegate_task(
  goal="Deep code audit of the project at /path/to/project. Review every file for bugs, security issues, and edge cases. Fix everything.",
  context="Project description and key files...",
  toolsets=["terminal", "file", "web"]
)
```

Subagent timeout is 600s — it may not finish large reviews but partial results are still useful. Check for changes with `git diff --stat` regardless of timeout.

## Merging Multiple Agent Reviews (Git Stash Pattern)

When multiple agents (Claude + subagent, or two Claude runs) produce overlapping diffs, merge them with git stash:
...
# Save Agent B's work to a branch or stash
git stash       # stash@{0} = Agent B, stash@{1} = Agent A

# Apply Agent A first
git stash pop stash@{1}

# Now manually cherry-pick non-overlapping improvements from Agent B
# (both stashes' diffs can be compared with git diff stash@{0}^..stash@{0})
```

This avoids the merge conflicts that `git stash apply` would hit and produces a clean combined result.
...
13. **ANTHROPIC_API_KEY alone does NOT authenticate `claude` CLI for print mode** — Claude Code v2.x checks OAuth first, regardless of the env var. Use `claude auth login --console` once to bind the key. Until then, `claude -p ...` will return "Not logged in" even with a valid key exported.
14. **Print mode with `pty=true` can hit the 600s Hermes timeout** — Claude may complete its work but the output doesn't flush to stdout before Hermes kills the process. Set `timeout=600` and check `git diff --stat` afterward; the changes are often written to disk even if Hermes reports a timeout.
15. **Claude Pro ($20/mo) does NOT work with Claude Code CLI** — `claude auth status` reports "logged in" with `subscriptionType: "pro"` and `authMethod: "claude.ai"`, but ALL API calls (both `-p` print mode AND interactive tmux mode) return 401 "Invalid authentication credentials". Every user message shows "Please run /login · API Error: 401". Claude Pro only works through the claude.ai browser chat — you need an API key (`ANTHROPIC_API_KEY` + `claude auth login --console`) or a Claude Max/Team/Enterprise subscription for CLI access. Always run the smoke test from the Pre-Flight Check to catch this before investing time in tmux orchestration. See `references/claude-pro-401-auth-failure.md` for full error transcripts.
16. **Non-Anthropic backends consistently time out with bare `ANTHROPIC_BASE_URL`** — Setting `ANTHROPIC_BASE_URL` to third-party proxies (Kimi via `api.kimi.com/coding`, GLM 5.1 via Z.AI) with `ANTHROPIC_API_KEY` set to the proxy key results in silent 30s timeouts with no output. Claude Code CLI v2.x performs startup checks (OAuth validation, plugin discovery) against the base URL that non-Anthropic backends don't handle. **SOLUTION for GLM/Z.AI:** Use the `@lee_ai/coding-helper` npm package (`chelper auth glm_coding_plan_global <key>` then `chelper auth reload claude-code`). This handles the backend protocol translation that bare `ANTHROPIC_BASE_URL` can't. Full setup: `references/glm-coding-plan-setup.md`. For other backends: fall back to building directly (write_file + delegate_task) or use Goose CLI (`goose`) which supports multi-provider backends natively. Same-day (2026-05-08) failures confirmed against: Kimi 2.6, GLM 5.1 (both general and coding Z.AI endpoints).
