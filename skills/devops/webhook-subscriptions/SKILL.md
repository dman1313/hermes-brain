...
7. **DNS record missing?** See `references/webhook-proxy-pattern.md` — piggyback on an existing subdomain with a Flask reverse-proxy route when you cannot add new DNS records.
  7. **Can't add DNS?** See `references/dns-less-proxy.md` for the app-proxy pattern — route webhooks through an existing subdomain when DNS is unavailable.
