...
15. **Caddy/Nginx basic auth loops with SPAs.** When protecting a tunnel-exposed SPA (like Hermes Dashboard) with Caddy `basic_auth`, the browser prompts for credentials, the proxy accepts them, but the backend's own 401 responses (from session-token checks or API auth middleware) re-trigger the browser's auth prompt — creating an infinite loop. Basic auth credentials are not automatically forwarded by the browser for XHR/fetch API calls in all cases, and backend 401s conflict with the proxy's auth layer. **Solution:** Use a lightweight Python auth proxy with cookie-based auth instead. The proxy serves a login page (no browser popup), sets an HttpOnly cookie on "click to enter", and proxies all subsequent requests carrying the cookie to the backend. No loops, no conflicts with the backend's own auth. See `references/auth-proxy-cookie.md` for a complete template.

16. **`cert.pem` exists but is corrupted.** Even if the file is present, `cloudflared tunnel route dns` may fail with `Error decoding origin cert: missing token in the certificate`. This means the cert file is incomplete or was not generated properly. Generate a fresh one from Cloudflare Dashboard (SSL/TLS → Origin Server → Create Certificate), or fall back to creating DNS records via the API token or Dashboard directly.
...
- `references/auth-proxy-cookie.md` — Cookie-based auth proxy for protecting SPAs behind tunnels (replaces Caddy basic auth which loops with SPAs).
