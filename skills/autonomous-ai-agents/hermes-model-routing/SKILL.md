...
- **deepseek-chat on Telegram gets interrupted during multi-step tool-calling** (observed May 13). When a Telegram session requires 3+ sequential tool calls (installing software, API setup, multi-step config), deepseek-chat's latency causes the Telegram delivery to time out, interrupting the session mid-response. The user has to restart from scratch. Prefer `mimo-v2.5-pro` or `qwen3.6-plus` for Telegram tool-heavy tasks — they're faster and less prone to interruption. Simple Q&A on Telegram is fine with any model.
