...
2. **Disk + memory + swap** — `df -h /` and `free -h` — flag if disk >80%, memory >85%, or **swap >90%**. Swap at ≥98% is RED: OOM killer risk. Swap creeping upward day-over-day (even if disk/RAM look fine) means a slow leak — zombie processes, k3s bloat, or unclosed file handles. Check `ps aux --sort=-%mem | head -5` for the top consumers. Quick cycle: `sudo swapoff -a && sudo swapon -a`. Permanent fix: increase swap file to 4G (`sudo fallocate -l 4G /swap.img && sudo mkswap /swap.img && sudo swapon /swap.img`).
...
- **Silent swap creep.** Swap fills gradually — disk and RAM can look fine while swap goes 73%→84%→99.98% over a week. The OOM killer doesn't warn before it strikes. Flag swap at >90% in every brief. Check `free -h | grep Swap` explicitly; don't assume `free -h` output alone will catch it. Top consumers: `ps aux --sort=-%mem | head -5`. This VPS has 7.6G RAM / 1.9G swap — the swap file is undersized for the workload (k3s alone uses 388MB resident).
