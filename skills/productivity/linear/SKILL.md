...
1. Get a personal API key from **Linear Settings > Account > Security & access > Personal API keys** (URL: https://linear.app/humangood/settings/account/security).

   ⚠️ **Do NOT use the org-level Settings > API page** — that page only shows OAuth apps and workspace-member keys. It asks for a callback URL and won't give you a personal bearer token. Go to your **personal account settings** (the URL above).
2. Set `LINEAR_API_KEY` in your environment:

```bash
# Option A — direct config set (fastest):
hermes config set LINEAR_API_KEY "lin_api_..."

# Option B — interactive setup walkthrough:
hermes setup
```

Note: `hermes config set env.LINEAR_API_KEY` will fail with `Invalid environment variable name`. Do NOT use the `env.` prefix — just the bare variable name.
...
Hermes receives Linear events via a webhook subscription. When issues are created, updated, or projects change, Linear POSTs to Hermes and triggers an agent run.

### Webhook URL
```
https://hermesdash.humangood.ai/webhooks/linear
```

### HMAC Secret
```
JCNU-ouTnYOGJCzU-f2XtJJTNXg_M7LQBcts4LBgNUQ
```

### Linear Webhook Setup
In Linear: **Settings → API → Webhooks → Create webhook**
- **URL:** `https://hermesdash.humangood.ai/webhooks/linear`
- **Secret:** the HMAC secret above
- **Events:** Issues (create, update), Projects (create, update)

### Architecture
The webhook routes through the hermesdash Flask proxy (`/opt/proxy-dashboard/dashboard.py`) which forwards `/webhooks/*` → `http://127.0.0.1:8644`. No separate DNS record needed.

### Hermes Side
Managed via `hermes webhook` CLI:
```bash
hermes webhook list              # check subscription status
hermes webhook test linear       # test with a sample payload
hermes webhook remove linear     # remove if needed
```

Config at `/home/ubuntu/.hermes/config.yaml` — webhook server on port 8644.
Cloudflare tunnel routes `webhook.humangood.ai` → `http://127.0.0.1:8644` in `/home/ubuntu/.cloudflared/config.yml`.
