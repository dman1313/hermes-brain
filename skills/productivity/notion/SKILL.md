...
The `/pages/{id}/markdown` endpoint requires a `type` field specifying the operation. The body shape depends on the type:

**`insert_content`** — append markdown to the page:
```bash
...
**`update_content`** — replace specific blocks (requires block IDs):
```bash
curl -s -X PATCH "https://api.notion.com/v1/pages/{page_id}/markdown" \
  -H "Authorization: Bearer $NOTION_API_KEY" \
  -H "Notion-Version: 2025-09-03" \
  -H "Content-Type: application/json" \
  -d '{"type": "update_content", "update_content": {"blocks": ["block_id_1"], "content": "New text"}}'
```

**`replace_content_range`** — replace a range of blocks.

The simple `{"markdown": "..."}` shape (without `type`) was deprecated and now returns 400: `body.type should be defined`. Always include the `type` and nest the markdown inside the type-specific key (e.g., `insert_content.content`).
...
### Create a database (inline on a page)

**Use API version 2022-06-28** — the 2025-09-03 endpoint silently drops custom properties
(see pitfall below). All other operations use 2025-09-03.
...
**Verification:** After creating, fetch the database and confirm `properties` is non-empty. If empty, delete (`PATCH` with `{"in_trash": true}`) and recreate with the 2022-06-28 header above.
...
### CRITICAL PITFALL: Database creation with v2025-09-03 silently drops properties

`POST /v1/databases` with `Notion-Version: 2025-09-03` returns 200 and creates a database, but **all custom select/multi-select/date properties are silently discarded.** Only the implicit title survives. Page creation then fails with `"Status is not a property that exists"`.

**Fix: Use API version 2022-06-28 for database creation only.** All other operations use 2025-09-03.

```bash
# CREATE database — use v2022-06-28
curl -s -X POST "https://api.notion.com/v1/databases" \
  -H "Authorization: Bearer $NOTION_API_KEY" \
  -H "Notion-Version: 2022-06-28" \
  -H "Content-Type: application/json" \
  -d '{"parent":{"type":"page_id","page_id":"xxx"},"is_inline":true,
       "title":[{"type":"text","text":{"content":"My DB"}}],
       "properties":{"Name":{"title":{}},"Status":{"select":{"options":[...]}}}}'

# ALL OTHER operations — use 2025-09-03 as normal
```

**Verification:** After creating, fetch the DB and check `properties` is non-empty. If empty: delete (`PATCH` with `"in_trash":true`) and recreate with 2022-06-28.
...
- API key: `ntn_640...` in `~/.hermes/.env` as `NOTION_API_KEY`
- ntn CLI is **not installed** on this VPS — use curl for all operations
- Dashboard page: `365ee95a-3df6-8088-ae29-d05dbccb6bcd`
- Inline databases on dashboard:
  - Site Development: `f4ecf36b`
  - Task Management Kanban: `8cfee95a-3df6-82ca-98a4-8119b20408a7`

### People Field Quirk

The People property only accepts real Notion users in the workspace (Dwayne + bot integration). For non-human assignees like agents, put `[Agent]` in the page **title** instead of trying to assign via the People field.
